...
- SVG doesn't have z-index. But because elements with `id`s end up "ripped out" of the SVG structure, we can put z-indices on them (with `dragologyZIndex`) to determine their ultimate ordering. Here, we use that to make sure the dragged tile is on top of the others.
      ...
      <g transform={translate(150, 150)}>
        <g transform={rotateDeg(state.angle)}>
          ...
          <circle
            ...
            r={20}
            fill="black"
            dragologyOnDrag={() => d.vary(state, [["angle"]])}
          />
        </g>
      </g>
    );
